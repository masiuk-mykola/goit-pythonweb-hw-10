...
class ContactRepository:
    ...
    async def get_contacts_by_ids(self, contact_id: list[int]) -> list[Contact]:
        stmt = select(Contact).where(Contact.id.in_(contact_id))
    ...
    async def get_contact_by_contact_info(
        ...
        stmt = select(Contact).filter_by(first_name=first_name.lower())
        contact = await self.db.execute(stmt)
        return contact.scalar_one_or_none()
